...
const AddEnrollment = () => {
  ...
  const { studentId_0, courseId_0, id, grade_mid, grade_fi } = useParams();
  ...
  const handleUpdateEnrollment = async () => {
    try {
      ...
      // await contract.methods.updateEnrollment(id, gradeMid, gradeFinal, courseId_0).send({ from: accounts[0] });


  //     contract.methods.updateEnrollment(id, gradeMid, gradeFinal, courseId_0)
  // .send({ from: accounts[0] }, function(error, transactionHash) {
  //   if (!error) {
  //     alert('Sửa điểm môn học thành công, vui lòng chờ trong giây lát... ')
  //     setTimeout(() => {
...
.on('transactionHash', function(transactionHash) {
    ...
    alert('Sửa điểm môn học thành công')
})
  ...
  return (
    <div style={{marginTop:'10px'}} className="add-enrollment-container">
      ...
      <div className="input-group">
        ...
        <input type="text"  onChange={(e) => setGradeMid(e.target.value)}  />
      </div>
      <div className="input-group">
        <label>TK:</label>
        <input type="text"  onChange={(e) => setGradeFinal(e.target.value)}  />
      </div>
      <button onClick={handleUpdateEnrollment} >Cập nhật</button>
    </div>
  );
};
